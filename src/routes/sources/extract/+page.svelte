...
<header class="head">
	...
	<p class="sub">
		Paste a newsletter, listings page, or any text with events. An LLM pulls out structured
		candidates you can tidy before publishing. Imported events are <Term id="proxied">proxied</Term>.
	</p>
</header>
...
{#if !ready}
	{#if data.capability}
		<div class="gate">
			<p class="gate-intro">This tool needs the LLM extraction capability:</p>
			<CapabilityGuide capability={data.capability} />
			<p class="gate-note">
				Set the key in your environment and redeploy — Studio ships none. Once it's present, this
				page becomes the extractor.
			</p>
		</div>
	{/if}
{:else}
		...
		use:enhance={() => {
			...
			return async ({ result }) => {
				...
				if (result.type === 'success' && result.data) {
					...
					toast.push(`Extracted ${candidates.length} events.`, 'success');
				} else if (result.type === 'failure') {
					toast.push(String(result.data?.error ?? 'Extraction failed.'), 'error');
				}
			};
		}}
	>
		<textarea
			name="text"
			rows="10"
			placeholder="Paste a newsletter or listings text here…"
		></textarea>
		<div class="row">
			<input type="url" name="sourceUrl" placeholder="Source URL (optional, for attribution)" />
			<input type="text" name="timezone" value="America/New_York" class="tz" aria-label="Default timezone" />
			<button type="submit" class="primary" disabled={extracting}>
...
<style>
	...
	.extract-form {
		max-width: 800px; display: flex; flex-direction: column; gap: 0.75rem; margin-bottom: 1.5rem;
		background: #fff; border: 1px solid #e5e5e5; border-radius: 8px; padding: 1rem;
	}
	.extract-form textarea {
		font-family: inherit; font-size: 0.9rem; padding: 0.6rem 0.75rem; border: 1px solid #d0d0d0;
		border-radius: 5px; outline: none; resize: vertical; line-height: 1.5;
	}
	.extract-form textarea:focus { border-color: #166534; box-shadow: 0 0 0 2px rgba(22, 101, 52, 0.15); }
	...
	.row .tz {
		width: 10rem; font-family: ui-monospace, SFMono-Regular, Menlo, monospace; font-size: 0.8rem;
		padding: 0.45rem 0.6rem; border: 1px solid #d0d0d0; border-radius: 5px; outline: none;
	}
	.row input:focus { border-color: #166534; box-shadow: 0 0 0 2px rgba(22, 101, 52, 0.15); }
	.publish-bar {
