...
describe('contactFromOsmTags', () => {
	it('reads website, preferring website over contact:website', () => {
		expect(contactFromOsmTags({ website: 'https://a.com', 'contact:website': 'https://b.com' }).website).toBe(
			'https://a.com',
		);
		expect(contactFromOsmTags({ 'contact:website': 'https://b.com' }).website).toBe('https://b.com');
	});
	...
	it('reads phone from either phone or contact:phone', () => {
		...
		expect(contactFromOsmTags({ 'contact:phone': '+1 215 555 0199' }).phone).toBe('+1 215 555 0199');
	});
