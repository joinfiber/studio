...
<header class="head">
	...
	<p class="sub">
		Paste a feed URL. Items become editable candidates. Imported events are <Term id="proxied">proxied</Term>
...
>
	...
	<button type="submit" class="primary" disabled={fetching}>{fetching ? 'Fetching…' : 'Fetch'}</button>
</form>

{#if candidates.length > 0}
	<p class="warn">
		Heads up: feeds carry a <strong>publish</strong> date, not an event date. Each candidate's time
		is seeded from the item's publish date — verify and correct it before publishing.
	</p>
	<form
		method="POST"
		action="?/publish"
...
<style>
	...
	.warn {
		max-width: 800px; margin: 0 auto 1rem; font-size: 0.82rem; color: #92400e;
		background: #fffbeb; border: 1px solid #fde68a; border-radius: 6px; padding: 0.6rem 0.85rem;
		line-height: 1.5;
	}
	.publish-bar {
