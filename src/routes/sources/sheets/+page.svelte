<script lang="ts">
	...
	const fields: { key: keyof SheetMapping; label: string; required?: boolean; hints: string[] }[] = [
		...
		{ key: 'description', label: 'Description', hints: ['description', 'desc', 'details', 'notes'] },
	];
	...
	function generate() {
		...
		if (result.candidates.length === 0) {
			...
			toast.push(`Generated ${result.candidates.length}, skipped ${result.skipped} (no name / bad date).`, 'info', 5000);
		} else {
...
>
	<input type="url" name="url" placeholder="https://docs.google.com/spreadsheets/d/.../edit" required />
	<input type="text" name="timezone" bind:value={timezone} class="tz" aria-label="Default timezone" />
...
{#if headers.length > 0}
	<section class="mapping">
		...
		<div class="map-grid">
			{#each fields as f}
				<label class="map-row">
					<span class="map-label">{f.label}{#if f.required}<em>*</em>{/if}</span>
...
<style>
	...
	.fetch-form { display: flex; gap: 0.5rem; margin-bottom: 1.5rem; max-width: 800px; }
	...
	.fetch-form input:focus { border-color: #166534; box-shadow: 0 0 0 2px rgba(22, 101, 52, 0.15); }
	.mapping {
		max-width: 800px; margin: 0 auto 1.5rem; padding: 1.25rem 1.5rem;
		background: #fff; border: 1px solid #e5e5e5; border-radius: 8px;
	}
	.mapping h3 { margin: 0 0 1rem; font-size: 0.95rem; font-weight: 600; color: #444; }
	.map-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 0.75rem 1rem; margin-bottom: 1.25rem; }
	.map-row { display: flex; flex-direction: column; gap: 0.3rem; font-size: 0.8rem; }
	.map-label { font-weight: 500; color: #666; text-transform: uppercase; letter-spacing: 0.05em; font-size: 0.7rem; }
	.map-label em { color: #991b1b; font-style: normal; }
	.map-row select {
		font-family: inherit; font-size: 0.9rem; padding: 0.4rem 0.6rem;
		border: 1px solid #d0d0d0; border-radius: 5px; background: #fff; outline: none;
	}
	.map-row select:focus { border-color: #166534; box-shadow: 0 0 0 2px rgba(22, 101, 52, 0.15); }
	.publish-bar {
