	...
	use:enhance={() => {
		...
		return async ({ result }) => {
			...
			if (result.type === 'success' && result.data) {
				...
				toast.push(`Parsed ${candidates.length} events.`, 'success');
			} else if (result.type === 'failure') {
				toast.push(String(result.data?.error ?? 'Import failed.'), 'error', 8000);
			}
		};
	}}
>
...
{#if candidates.length > 0}
	...
	>
		<input
			...
			placeholder="Organizer for these events (e.g. Johnny Brenda's)"
			required
		/>
